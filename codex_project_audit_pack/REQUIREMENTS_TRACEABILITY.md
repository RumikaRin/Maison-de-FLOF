...
| REVIEW-01 | Đánh giá sản phẩm đã mua | Customer | product detail | `GET/POST /api/reviews`, review workflow service | Review, Order, OrderItem, User, Notification, AuditLog | PostgreSQL verified-purchase/upsert/staff-reply/delete integration | Đạt có giới hạn | Service được route sử dụng đã chứng minh; chưa browser E2E |
...
| QUOTE-01 | Gửi và xử lý báo giá | Guest/Staff | `/quote-request`, `/admin/quotes` | quote workflow service dùng bởi public/admin route | QuoteRequest, Notification, AuditLog | PostgreSQL create/notify/status/audit integration | Đạt có giới hạn | Chưa anti-spam/browser E2E |
| CHAT-01 | Chat hỗ trợ | Guest/Customer/Staff | global chat, `/admin/chat` | legacy chat + authenticated conversation workflow service | ChatMessage, Conversation, Message, Notification, AuditLog | PostgreSQL user-scope/reopen/staff-reply/read integration | Một phần | Conversation mới yêu cầu login; legacy/public model song song và notification có thể spam |
...
| ADMIN-02 | CRUD catalog/supplier | Admin | `/admin/catalog` | admin category service/routes; supplier cùng policy class | Category, Supplier, AuditLog | category create/duplicate/update/deactivate/audit DB integration + 59-method policy unit | Đạt có giới hạn | Supplier chưa có direct HTTP integration riêng |
...
| TEST-01 | Automated quality gate | Developer | N/A | GitHub Actions | PostgreSQL 18 test service | 81 unit + 15 DB integration + 13 E2E/axe + OpenAPI + Lighthouse | Đạt có giới hạn | 59 admin method có policy inventory; chưa có coverage threshold |
...
`CHECK-02` vẫn chưa đạt nhưng đã được người dùng loại khỏi phạm vi vì VNPay chỉ là giả lập. Với phạm vi còn lại, register/reset, checkout COD, ownership, outbox, audit persistence, catalog transaction, review/quote/authenticated-chat, admin policy, session middleware, accessibility và performance đã có bằng chứng tự động. Khoảng trống lớn nhất còn lại là direct HTTP breadth cho CRUD tương đồng và bằng chứng hạ tầng/provider production trực tiếp.
