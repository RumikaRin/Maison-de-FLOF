...
| REVIEW-01 | Đánh giá sản phẩm đã mua | Customer | product detail | `GET/POST /api/reviews`, review workflow service | Review, Order, OrderItem, User, Notification, AuditLog | PostgreSQL workflow integration + HTTP E2E chặn user chưa mua | Đạt có giới hạn | Rule verified purchase đã chạy qua route thật; chưa E2E cho review hợp lệ sau đơn completed |
...
| QUOTE-01 | Gửi và xử lý báo giá | Guest/Staff | `/quote-request`, `/admin/quotes` | quote workflow service dùng bởi public/admin route | QuoteRequest, Notification, AuditLog | PostgreSQL integration + public create/ADMIN update HTTP E2E | Đạt có giới hạn | Persistence và audit đã chạy qua route thật; chưa anti-spam/load test |
| CHAT-01 | Chat hỗ trợ | Guest/Customer/Staff | global chat, `/admin/chat` | legacy chat + authenticated conversation workflow service | ChatMessage, Conversation, Message, Notification, AuditLog | PostgreSQL integration + customer/ADMIN reply/read HTTP E2E | Đạt có giới hạn | Authenticated conversation đã chứng minh xuyên tầng; legacy/public model song song và notification có thể spam |
...
| ADMIN-02 | CRUD catalog/supplier | Admin | `/admin/catalog` | admin category service/routes; supplier cùng policy class | Category, Supplier, AuditLog | category DB integration + HTTP create/duplicate/update/deactivate/audit + 59-method policy unit | Đạt có giới hạn | Category và role denial đã chứng minh qua HTTP; supplier chưa có direct HTTP integration riêng |
| ADMIN-03 | CRUD sơn/khuyến mãi | Admin | `/admin/paints` | admin product service/routes, `/promotions` | Paint, PaintColorLink, InventoryTransaction, AuditLog | product create/duplicate/color-replace/deactivate atomic DB integration + policy unit | Đạt có giới hạn | Promotion chưa có direct DB/HTTP test |
| ADMIN-04 | CRUD màu/bộ sưu tập | Admin | `/admin/colors`, `/admin/collections` | color service/routes; collection cùng policy class | PaintColor, ColorCollection, PaintColorLink, AuditLog | linked-color delete guard/audit DB integration + policy unit | Đạt có giới hạn | Collection chưa có direct HTTP integration riêng |
...
| NOTIF-01 | Thông báo admin | Staff/Admin | dropdown admin | admin notification APIs | Notification, AuditLog | pagination + audit sanitizer + admin guard-policy unit | Một phần | Đã chuẩn hóa `requireStaff` và ownership; chưa realtime/API integration |
...
| TEST-01 | Automated quality gate | Developer | N/A | GitHub Actions | PostgreSQL 18 test service | 83 unit + 15 DB integration + 19 E2E/axe + OpenAPI 99/99 + Lighthouse | Đạt có giới hạn | Actions checkout/setup-node v5 và 59 admin method có policy inventory; chưa có coverage threshold |
...
`CHECK-02` vẫn chưa đạt nhưng đã được người dùng loại khỏi phạm vi vì VNPay chỉ là giả lập. Với phạm vi còn lại, register/reset, checkout COD, ownership, outbox, audit persistence, category catalog, review denial, quote/authenticated-chat, admin policy, session middleware, accessibility và performance đã có bằng chứng tự động. OpenAPI đối chiếu đủ 52 route file/99 operation. Khoảng trống lớn nhất còn lại là direct HTTP breadth cho CRUD tương đồng và bằng chứng hạ tầng/provider production trực tiếp.
