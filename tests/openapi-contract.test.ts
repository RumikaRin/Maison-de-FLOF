...
test("API inventory discovers every route file and exported operation", async () => {
  ...
  assert.equal(new Set(inventory.map(({ file }) => file)).size, 68);
  ...
  assert.ok(
    inventory.some(
      ({ path, method }) =>
        path === "/api/visualizer/designs/{id}" && method === "delete",
    ),
  );
  assert.ok(
    inventory.some(
...
test("OpenAPI covers every source operation and reusable contract schema", async () => {
  ...
  assert.equal(sourceOperations.length, 120);
  assert.equal(documentedOperations.length, 120);
});
