...
import { clampVelocity, decayVelocity } from "../src/lib/fl-slice.ts";
...
test("decayVelocity lerps toward target and snaps to zero below epsilon", () => {
  ...
  // snap must not fire while the target is still live
  assert.notEqual(decayVelocity(0.0005, 0.5), 0);
  assert.equal(decayVelocity(-0.0005, 0), 0);
  let v = 1;
